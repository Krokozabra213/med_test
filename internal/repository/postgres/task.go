...
func (r *Repository) CreateTask(ctx context.Context, task *taskdomain.Task) (*taskdomain.Task, error) {
	...
	if err != nil {
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.CreateTask: query failed: %w", err))
	}
	defer rows.Close()
...
func (r *Repository) GetTaskByID(ctx context.Context, id int64) (*taskdomain.Task, error) {
	...
	rows, err := r.pool.Query(ctx, query, args)
	if err != nil {
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.GetTaskByID: query failed: %w", err))
	}
	defer rows.Close()
	...
	if err != nil {
		...
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.GetTaskByID: collect row failed: %w", err))
	}
...
func (r *Repository) UpdateTask(ctx context.Context, task *taskdomain.Task) (*taskdomain.Task, error) {
	...
	rows, err := r.pool.Query(ctx, query, args)
	if err != nil {
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.UpdateTask: query failed: %w", err))
	}
	defer rows.Close()
...
func (r *Repository) DeleteTask(ctx context.Context, id int64) error {
	...
	if err != nil {
		return domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.DeleteTask: execute failed: %w", err))
	}

	if result.RowsAffected() == 0 {
		return domain.NewNotFound(taskEntity)
	}
...
func (r *Repository) ListTask(ctx context.Context) ([]taskdomain.Task, error) {
	...
	if err != nil {
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.ListTask: query failed: %w", err))
	}
	defer rows.Close()

	tasks, err := pgx.CollectRows(rows, pgx.RowToStructByName[taskdomain.Task])
	if err != nil {
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.ListTask: collect rows failed: %w", err))
	}

	return tasks, nil
}

func (r *Repository) CreateTaskWithTx(
	ctx context.Context,
	tx pgx.Tx,
	...
	if err != nil {
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.CreateTaskWithTx: query failed: %w", err))
	}
	defer rows.Close()
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.CreateTaskWithTx: collect row not found: %w", err))
		}
		return nil, domain.NewInternal(domain.MessageInternal, fmt.Errorf("repository.CreateTaskWithTx: collect row failed: %w", err))
	}
